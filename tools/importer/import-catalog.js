...
const PAGE_TEMPLATE = {
  "name": "catalog",
  "description": "Dentsply Sirona en-us deep pages (category landings, brand pages, learn education, support, product topics). Reuses the same block set as the top-level template; adds the AEM Core .cmp-carousel card carousel (mapped to carousel-promo).",
  "urls": [
    "https://www.dentsplysirona.com/en-us/explore/digital-dentistry.html"
  ],
  "blocks": [
    {
      "name": "hero-home",
...
export default {
  ...
  transform: (payload) => {
    ...
    [...rebuilt.querySelectorAll('h1, h2, h3, h4, h5, h6')].forEach((h) => {
      ...
      // A heading nested inside a block table (e.g. the hero title, which lives
      // in a hero-home cell) must never be treated as an orphan — doing so
      // deleted the title of no-image heroes (category / learn / support pages),
      // leaving an empty hero.
